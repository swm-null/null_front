...
const useSearchConversationManager = () => {
  ...
  const [searchConversations, setSearchConversations] = useState<
    MemoSearchConversation[]
  >(() => JSON.parse(localStorage.getItem('search_queries') || '[]'));
  ...
  const addSearchConversation = (text: string) => {
    ...
    setSearchConversations((prevViews) => [
      ...
      ...(prevViews.length > MAX_SEARCH_QUERIES
        ? prevViews.slice(0, -1)
        : prevViews),
    ]);
    return answerID;
  ...
  const editSearchConversation = (
    id: string,
    newMemoSearchAnswer: MemoSearchAnswer
  ) => {
    setSearchConversations((prevViews) =>
      prevViews.map((view) =>
        view.id === id ? { ...view, answer: newMemoSearchAnswer } : view
      )
    );
    return id;
  };
  ...
  const removeSearchConversation = (id: string) => {
    setSearchConversations((prevViews) =>
      prevViews.filter((view) => view.id !== id)
    );
    return id;
  };
...
export default useSearchConversationManager;
