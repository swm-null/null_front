...
import {
  getSearchHistories,
  isSearchHistoriesResponse,
  isSearchInitResponse,
  isSearchMemoWithAIResponse,
  isSearchMemoWithDBResponse,
  paginationSearchHistories,
  searchMemo,
  searchMemoWithDB as searchMemoWithDBApi,
  searchMemoWithAI as searchMemoWithAIApi,
} from './api';
...
interface SearchQueryData {
  pages: paginationSearchHistories[];
  pageParams: number[];
}
...
const useSearchMemoManager = () => {
  ...
  >({
    ...
    queryFn: async ({ pageParam = 1 }: any) => {
      ...
      if (!isSearchHistoriesResponse(response)) {
        throw new Error(t('pages.search.fetchHistoryErrorMessage'));
      }

      return response as paginationSearchHistories;
    },
  ...
  const handleSearchInit = async (query: string) => {
    ...
    if (!isSearchInitResponse(response)) {
  ...
  const searchMemoInit = async (query: string) => {
    const response = await searchMemo(query);
    if (!isSearchInitResponse(response)) {
      throw new Error('Memo Search Error');
    }
    return response;
  ...
  const handleSearchWithDB = async (searchHistoryId: string) => {
    try {
      ...
      if (!isSearchMemoWithDBResponse(response)) {
        throw new Error('Memo Search Error');
      }
  ...
  const searchMemoWithDB = async (searchHistoryId: string) => {
    const response = await searchMemoWithDBApi(searchHistoryId);
    if (!isSearchMemoWithDBResponse(response)) {
      throw new Error('Memo Search Error');
    }
    return response;
  ...
  const handleSearchWithAI = async (searchHistoryId: string) => {
    try {
      ...
      if (!isSearchMemoWithAIResponse(response)) {
        throw new Error('Memo Search Error');
      }
  ...
  const searchMemoWithAI = async (searchHistoryId: string) => {
    const response = await searchMemoWithAIApi(searchHistoryId);
    if (!isSearchMemoWithAIResponse(response)) {
      throw new Error('Memo Search Error');
    }
    return response;
